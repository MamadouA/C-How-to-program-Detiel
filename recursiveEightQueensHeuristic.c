...
int chooseNextSquare( size_t row, size_t column, int chess[][ SIZE ], int eliminations[][ SIZE ] );
...
int solveNQ( int chessboard[][ SIZE ] ) {
    ...
    for( iQueen = 2; iQueen <= QUEENS; ++iQueen ) {
        ...
        chooseNextSquare( 0, 0, chessboard, eliminationNumbers );
...
int chooseNextSquare( size_t row, size_t column, int chess[][ SIZE ], int eliminations[][ SIZE ] ) {
    ...
    if( ( row < SIZE - 1 ) && ( column == SIZE - 1 ) ) {
        return chooseNextSquare( ++row, 0, chess, eliminations ); // recursive steps
    } // end if

    if( isSafe( row, column, row + column, row - column, chess ) ) { // if the current square is not attacked
        // compare the elimination numbers
        ...
        done = 1; // if there is a safe square to place a queen
    } // end outer if

    // recursive steps
    return chooseNextSquare( row, ++column, chess, eliminations );
